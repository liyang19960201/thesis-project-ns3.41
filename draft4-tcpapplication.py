...
maxBytes = 1024*1024*30  # Set to 0 for unlimited data transfer
filepath="/file" 
...
source = ns.applications.BulkSendHelper('ns3::TcpSocketFactory', 
                                        ns.network.InetSocketAddress(stainterface.GetAddress(3), port).ConvertTo())
# Source node (node 0) will send TCP traffic to the destination node (node 1)
...
# ###TCP section-2
port1=8080
maxBytes1 = 1024*1024*10

# Source node (node 0) will send TCP traffic to the destination node (node 1)
source = ns.applications.BulkSendHelper('ns3::TcpSocketFactory', 
                                        ns.network.InetSocketAddress(stainterface.GetAddress(0), port1).ConvertTo())
# Source node (node 0) will send TCP traffic to the destination node (node 1)
source.SetAttribute("MaxBytes", ns.core.UintegerValue(maxBytes1))
