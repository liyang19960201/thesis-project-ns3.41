...
def read_file(filename):
    with open(filename, "rb") as file:
        return file.read()
file_data = read_file("example.pdf")
...
# Define a function to log node positions
# Define a function to log node positions
def log_node_positions(node):
    position = node.GetPosition()
    

# Attach the callback function to the mobility model of each node
def logNodePosition(nodeContainer : ns.NodeContainer, showLegend: bool = True) -> None:
...
# echoClient = ns.applications.TcpEchoClientHelper(serveraddr,9)
...
port = 5002  # Use any unused port number
maxBytes = 4096  # Set to 0 for unlimited data transfer

# Source node (node 0) will send TCP traffic to the destination node (node 1)
source = ns.applications.BulkSendHelper('ns3::TcpSocketFactory', ns.network.InetSocketAddress(stainterface.GetAddress(3), port).ConvertTo())
# Source node (node 0) will send TCP traffic to the destination node (node 1)
source.SetAttribute("MaxBytes", ns.core.UintegerValue(maxBytes))

sourceApps = source.Install(wifiStaNodes.Get(0))
sourceApps.Start(ns.core.Seconds(2.0))  # Start sending TCP traffic after 1 second
sourceApps.Stop(ns.core.Seconds(10.0))  # Stop after 10 seconds


# Create a TCP receiver on the destination node
sink = ns.applications.PacketSinkHelper('ns3::TcpSocketFactory', ns.network.InetSocketAddress(stainterface.GetAddress(3), port).ConvertTo())
sinkApps = sink.Install(wifiStaNodes.Get(3))
sinkApps.Start(ns.core.Seconds(1.0))  # Start receiving TCP traffic immediately
sinkApps.Stop(ns.core.Seconds(10.0))  # Stop after 10 seconds
...
# Source node (node 0) will send TCP traffic to the destination node (node 1)
source = ns.applications.BulkSendHelper('ns3::TcpSocketFactory', ns.network.InetSocketAddress(stainterface.GetAddress(0), port1).ConvertTo())
# Source node (node 0) will send TCP traffic to the destination node (node 1)
source.SetAttribute("MaxBytes", ns.core.UintegerValue(maxBytes))
sourceApps = source.Install(wifiStaNodes.Get(3))
...
# Create a TCP receiver on the destination node
sink = ns.applications.PacketSinkHelper('ns3::TcpSocketFactory', ns.network.InetSocketAddress(stainterface.GetAddress(0), port1).ConvertTo())
sinkApps = sink.Install(wifiStaNodes.Get(0))
sinkApps.Start(ns.core.Seconds(1.0))  # Start receiving TCP traffic immediately
sinkApps.Stop(ns.core.Seconds(10.0))  # Stop after 10 seconds
...
#This section only execute if tracing is enabled
    ...
    phy.EnablePcap("draft", apDevices.Get(0),True)
